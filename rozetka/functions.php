...
?>
 <div >
    ...
    <?php endif ?>
        ...
        $to = $current_page + 3;
        if($to > $last_page) $to = $last_page;
        if($last_page - $current_page > 3) $right_prefix = "<li class='page-item'><span class='page-link'>...</span></li><li class='page-item'><a href='$link_last_page' class='page-link'>$last_page</a></li>";
        if($last_page - $current_page == 4) $right_prefix = "<li class='page-item'><a class='page-link' href='$link_last_page'>$last_page</a></li>";
        echo $left_prefix;
        for ($i=$from; $i < $to; $i++): 
            // $new_offset = ($i) * $limit;
            $link = query_add(['offset'=> $i * $limit]);
...
{
    $image = resizeImage($input, $new_size, $new_size);
